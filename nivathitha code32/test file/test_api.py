	...
	def post(self):
		...
		for i in range(0,len(data_file)):
			...
			for k in range(0,len(uniq_key)):
				...
				#print(res_str)
				#if uniq_key[k] not in( 'job_name','Date','Material','component_status_1'):
				if res:
		...
		res = template.render(col_name=col_name,df1_len=df1_len,df2_len=df2_len,component=component,
			...
			alarmmsg=alarmmsg,compstatus=compstatus,other=other,mergedata=mergedata,mergelen=mergelen)

		text_file = open("index.html", "w")
		text_file.write(res)
		text_file.close()
		path = os.path.abspath('index.html')
		converter.convert(path,file_name+'.pdf')
